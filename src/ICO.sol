...
contract ICO {
    ...
    modifier withinPhaseLimits(uint256 amount) {
        if (currentPhase == Phase.SEED) {
            require(userToContributedAmount[msg.sender] + amount <= SEED_INDIVIDUAL_LIMIT, "Exceeds individual SEED phase limit");
            require(totalContributions + amount <= SEED_TOTAL_LIMIT, "Exceeds SEED phase limit");
            
        } else if (currentPhase == Phase.GENERAL) {
            require(userToContributedAmount[msg.sender] + amount <= GENERAL_INDIVIDUAL_LIMIT, "Exceeds individual GENERAL phase limit");
